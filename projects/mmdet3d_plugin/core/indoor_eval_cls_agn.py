...
def eval_map_recall_cls_agn(pred, gt, ovthresh=None):
    ...
    for label in gt.keys():
        for iou_idx, thresh in enumerate(ovthresh):
            for classname in gt[label].keys():
                if label in pred:
                    recall[iou_idx][label], precision[iou_idx][label], ap[iou_idx][
                        label] = ret_values[label][classname][iou_idx]
                else:
...
def assign_cls_by_centroid(pred, gt):
    """Assign predicted boxes to closest GT by centroid distance.

    Args:
        pred (dict): Detection results mapping image_id to predicted boxes.
        gt (dict): Ground truths mapping class_id to image_id and GT boxes.

    Returns:
        dict: Predictions assigned to GT classes, structured as class_id -> img_id -> predictions.
    """
    assigned_preds = {}

    for class_id, pred_boxes_by_img in pred.items():
        for img_id, pred_boxes in pred_boxes_by_img.items():
            for pred_box in pred_boxes:
                pred_center = pred_box[0].tensor[0][:3]
                min_dist = float('inf')
                assigned_class = None
                for gt_class_id in list(gt.keys()):
                    if img_id not in gt[gt_class_id]:
                        continue
                    for gt_boxes in gt[gt_class_id][img_id]:
                        for gt_box in gt_boxes:
                            gt_center = gt_box[:3]
                            dist = np.linalg.norm(pred_center - gt_center)
                            if dist < min_dist:
                                min_dist = dist
                                assigned_class = gt_class_id
                        
                if assigned_class is None:
                    continue
                if assigned_class not in assigned_preds:
                    assigned_preds[assigned_class] = {}
                if img_id not in assigned_preds[assigned_class]:
                    assigned_preds[assigned_class][img_id] = []
                assigned_preds[assigned_class][img_id].append(pred_box)

    return assigned_preds

def eval_map_recall_cls_agn_yc(pred, gt, ovthresh=None):
    assigned_preds = assign_cls_by_centroid(pred, gt)

    ret_values = {}
    for classname in gt.keys():
        if classname in assigned_preds:
            ret_values[classname] = eval_det_cls(assigned_preds[classname],
                                                 gt[classname], ovthresh)
    recall = [{} for i in ovthresh]
    precision = [{} for i in ovthresh]
    ap = [{} for i in ovthresh]
...
def indoor_eval_ov_cls_agn(
    ...
    gt = {}  # map {class_id: gt}
    gt_cls_specific = {} # map {class_id: gt}
    for img_id in range(len(dt_annos)):
        ...
        if gt_anno['gt_num'] != 0:
            ...
            labels_cls_agn_3d = np.ones_like(labels_3d)
        else:
            gt_boxes = box_type_3d(np.array([], dtype=np.float32))
            labels_3d = np.array([], dtype=np.int64)
            labels_cls_agn_3d = np.array([], dtype=np.int64)
        ...
        #     label = labels_3d[i]
        #     bbox = gt_boxes[i]
        #     if label not in gt_cls_specific:
        #         gt_cls_specific[label] = {}
        #     if img_id not in gt_cls_specific[label]:
        #         gt_cls_specific[label][img_id] = []
        #     gt_cls_specific[label][img_id].append(bbox)

        for i in range(len(labels_cls_agn_3d)):
            label = labels_cls_agn_3d[i]
            bbox = gt_boxes[i]
            #  label_cls_specific = labels_3d[i]
            label_cls_specific = 1
            if label not in gt:
                gt[label] = {}
            if label_cls_specific not in gt[label]:
                gt[label][label_cls_specific] = {}
            if img_id not in gt[label][label_cls_specific]:
                gt[label][label_cls_specific][img_id] = []
            gt[label][label_cls_specific][img_id].append(bbox)

    rec, prec, ap = eval_map_recall_cls_agn(pred, gt, metric)
    ret_dict = dict()
    header = ['classes']
...
def indoor_eval_ov_cls_agn_yc(
    ...
    rec, prec, ap = eval_map_recall_cls_agn_yc(pred, gt, metric)
    ret_dict = dict()
    header = ['classes']
