...
def indoor_eval_ov(
    ...
    if len([ap[0][i] for i in unseen_ids]) > 0 and len([ap[0][i] for i in seen_ids]) > 0:
        ...
        unseen_aps_25 = np.concatenate(
            [ap[0][i] for i in unseen_ids if not np.isnan(ap[0][i])] 
        ) if any(not np.isnan(ap[0][i]) for i in unseen_ids) else np.array([])
...
def eval_det_cls_pklpkl(pred, gt, iou_thr=None):
    """Generic functions to compute precision/recall for object detection for a
    single class.
...
def eval_map_recall_pklpkl(pred, gt, ovthresh=None):
    """Evaluate mAP and recall.
    ...
    for classname in gt.keys():
        if classname in pred:
            ret_values[classname] = eval_det_cls_pklpkl(pred[classname],
                                                 gt[classname], ovthresh)
    recall = [{} for i in ovthresh]
    precision = [{} for i in ovthresh]
    ap = [{} for i in ovthresh]
...
def indoor_eval_ov_pklpkl(
    ...
    for img_id in range(len(dt_annos)):
        ...
        if dt_anno['gt_num'] != 0:
            dt_boxes = box_type_3d(
                dt_anno['gt_boxes_upright_depth'],
                box_dim=dt_anno['gt_boxes_upright_depth'].shape[-1],
                origin=(0.5, 0.5, 0.5)).convert_to(box_mode_3d)
            # replace the original real length and width
            if axis_aligned_lw:
                corner3d = dt_boxes.corners
                minmax_box3d = corner3d.new(torch.Size((corner3d.shape[0], 6)))
                minmax_box3d[:, :3] = torch.min(corner3d, dim=1)[0]
                minmax_box3d[:, 3:] = torch.max(corner3d, dim=1)[0]
                dims = minmax_box3d[:, 3:] - minmax_box3d[:, :3]
                boxes = dt_anno['gt_boxes_upright_depth'].copy()
                boxes[:, 3:6] = dims
                dt_boxes = box_type_3d(
                    boxes,
                    box_dim=dt_anno['gt_boxes_upright_depth'].shape[-1],
            ...
            labels_3d = dt_anno['class']
        else:
            dt_boxes = box_type_3d(np.array([], dtype=np.float32))
            labels_3d = np.array([], dtype=np.int64)
    ...
    rec, prec, ap = eval_map_recall_pklpkl(pred, gt, metric)
    ret_dict = dict()
    header = ['classes']
    ...
    for i, iou_thresh in enumerate(metric):
        ...
        for label in rec[i].keys():
            if len(rec[i][label]) == 0:
                rec[i][label] = np.array([0.])
            ret_dict[f'{label2cat[label]}_rec_{iou_thresh:.2f}'] = float(
                rec[i][label][-1])
            rec_list.append(rec[i][label][-1])
        ret_dict[f'mAR_{iou_thresh:.2f}'] = float(np.mean([i for i in rec_list if not np.isnan(i)])) #float(np.mean(rec_list))
